...
const s = StyleSheet.create({
  ...
  quickWinBox: {
    backgroundColor: '#ecfeff', borderRadius: 6,
    padding: 10, marginBottom: 8, marginTop: 4,
  },
  quickWinItem: { flexDirection: 'row', marginBottom: 4 },
  quickWinDot: { fontSize: 9, color: C.cyan, marginRight: 6, fontFamily: 'Helvetica-Bold' },
  quickWinText: { fontSize: 9, lineHeight: 1.55, flex: 1, color: C.slate },
  actionRow: {
    flexDirection: 'row', alignItems: 'flex-start',
    backgroundColor: C.light, padding: 8, borderRadius: 4, marginBottom: 5,
  },
  actionText: { fontSize: 9, flex: 1, lineHeight: 1.5, color: C.slate, marginRight: 8 },
  actionPills: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'flex-start', maxWidth: 140 },
  actionPill: {
  ...
  benchmarkCallout: {
    backgroundColor: '#ecfeff', borderLeftWidth: 4, borderLeftColor: C.cyan,
    padding: 12, borderRadius: 4, marginBottom: 16,
  },
  benchmarkText: { fontSize: 11, color: C.slate, fontStyle: 'italic', lineHeight: 1.6 },
  gapRow: {
    flexDirection: 'row', backgroundColor: C.light,
    padding: 10, borderRadius: 6, marginBottom: 8,
  },
  gapNum: {
    width: 20, height: 20, borderRadius: 10, backgroundColor: C.cyan,
    justifyContent: 'center', alignItems: 'center', marginRight: 10, flexShrink: 0,
  },
  gapNumText: { fontSize: 9, fontFamily: 'Helvetica-Bold', color: C.white },
  gapText: { fontSize: 9, flex: 1, lineHeight: 1.5, color: C.slate },
  roadmapPhase: {
    marginBottom: 16, borderTopWidth: 3, borderTopColor: C.cyan,
    backgroundColor: C.light, padding: 12, borderRadius: 4,
  },
  roadmapPhaseTitle: { fontSize: 10, fontFamily: 'Helvetica-Bold', marginBottom: 8, letterSpacing: 0.5 },
  ...
  checklistItem: {
    flexDirection: 'row', alignItems: 'flex-start',
    paddingVertical: 5, borderBottomWidth: 0.5, borderBottomColor: C.border,
  },
  checkbox: {
    width: 10, height: 10, borderWidth: 1, borderColor: C.muted,
    borderRadius: 2, marginRight: 8, marginTop: 1, flexShrink: 0,
  },
  checklistDimLabel: {
    fontSize: 7, fontFamily: 'Helvetica-Bold', color: C.cyan,
    width: 56, marginRight: 6, marginTop: 2,
  },
  checklistText: { fontSize: 9, flex: 1, lineHeight: 1.5, color: C.slate, marginTop: 1 },
  toolRow: {
    flexDirection: 'row', backgroundColor: C.light,
    padding: 10, borderRadius: 6, marginBottom: 6,
  },
  toolBullet: {
    width: 4, height: 4, borderRadius: 2, backgroundColor: C.cyan,
    marginRight: 10, marginTop: 6,
  },
  toolText: { fontSize: 9, flex: 1, lineHeight: 1.5, color: C.slate },
  disclaimer: {
    marginTop: 18, padding: 10, backgroundColor: C.light, borderRadius: 4,
    fontSize: 8, color: C.muted, lineHeight: 1.6, fontStyle: 'italic',
  },
});
...
const PageHeader = ({ pageNum, totalPages }) => (
  <View style={s.header}>
    ...
    <Text style={s.pageNum}>PAGE {pageNum} OF {totalPages}</Text>
  </View>
);
...
const ActionPill = ({ label, level }) => (
  <View style={{ ...s.actionPill, backgroundColor: `${effortImpactColor(level)}20` }}>
...
const DimensionPage = ({ pageNum, totalPages, url, date, dim }) => {
  ...
  const hasActions = Array.isArray(dim.prioritizedActions) && dim.prioritizedActions.length > 0;
            ...
            <View key={i} style={s.actionRow} wrap={false}>
              ...
              <View style={s.actionPills}>
                ...
                {a.estimatedTrafficLift && (
                  <View style={{ ...s.actionPill, backgroundColor: `${C.cyan}20` }}>
                    <Text style={{ ...s.actionPillText, color: C.cyan }}>{a.estimatedTrafficLift}</Text>
                  </View>
                )}
              </View>
            </View>
  ...
  return (
    <Document title={`CiteSite Audit — ${url}`} author="CiteSite">
      ...
      <Page size="A4" style={s.page} wrap>
        ...
        {dimensions.map((d) => (
          <View key={d.id} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }} wrap={false}>
          ...
          <View style={{ marginBottom: 16 }}>
            ...
            {criticalIssues.slice(0, 3).map((issue, i) => (
              <View key={i} style={s.issueBox} wrap={false}>
                ...
                {issue.fix && <Text style={{ ...s.issueBody, color: C.slate }}>{issue.fix}</Text>}
                {issue.codeSnippet && (
                  <View style={s.codeBlock}>
                    <Text style={s.codeText}>{issue.codeSnippet.slice(0, 400)}</Text>
                  </View>
                )}
              </View>
            ))}
              ...
              <View key={i} style={s.improveBox} wrap={false}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={{ flexDirection: 'row', marginTop: 4, flexWrap: 'wrap' }}>
                    {item.impact && <Text style={{ fontSize: 7, color: C.muted, marginRight: 8 }}>Impact: {item.impact}</Text>}
                    {item.effort && <Text style={{ fontSize: 7, color: C.muted, marginRight: 8 }}>Effort: {item.effort}</Text>}
                    {item.estimatedTrafficLift && <Text style={{ fontSize: 7, color: C.cyan, fontFamily: 'Helvetica-Bold' }}>{item.estimatedTrafficLift}</Text>}
                  </View>
                </View>
              </View>
            ))}
      ...
      {competitorInsights && (
        <Page size="A4" style={s.page} wrap>
          ...
          <View style={s.accent} />
          <Text style={s.subtitle}>How this page compares to the industry benchmark</Text>
      ...
      {roadmap && (
        <Page size="A4" style={s.page} wrap>
          ...
          <View style={s.accent} />
          <Text style={s.subtitle}>Phased action plan, ordered by priority</Text>
          ...
          {[
            ...
            <View key={phase.label} style={{ ...s.roadmapPhase, borderTopColor: phase.color }} wrap={false}>
              <Text style={{ ...s.roadmapPhaseTitle, color: phase.color }}>{phase.label}</Text>
              {(phase.items || []).map((item, i) => (
                <View key={i} style={s.roadmapItem}>
                  <Text style={{ ...s.roadmapBullet, color: phase.color }}>▸</Text>
      ...
      {allQuickWins.length > 0 && (
        <Page size="A4" style={s.page} wrap>
          ...
          <View style={s.accent} />
          <Text style={s.subtitle}>Each item below is implementable in under one working day. Tick them off as you ship.</Text>
          ...
          {allQuickWins.map((item, i) => (
            <View key={i} style={s.checklistItem} wrap={false}>
              ...
              <Text style={s.checklistText}>{item.win}</Text>
            </View>
          ))}
      ...
      {toolRecommendations?.length > 0 && (
        <Page size="A4" style={s.page} wrap>
          ...
          <View style={s.accent} />
          <Text style={s.subtitle}>Specialist tooling tied to issues found on this page</Text>
          ...
          <View style={s.disclaimer}>
            <Text>
              This report was generated by CiteSite using AI-assisted analysis. Findings are
              indicative and should be validated with specialist tooling and human judgement
              before making significant technical or content changes. Domain Authority,
              backlink and traffic estimates require dedicated link-graph and analytics
