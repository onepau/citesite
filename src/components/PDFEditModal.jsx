...
export function PDFEditModal({
    ...
    setDraft((prev) => ({
      ...
      dimensions: prev.dimensions.map((d, i) =>
        i === dimIndex
          ? {
              ...d,
              observations: d.observations.map((o, j) =>
                j === obsIndex ? value : o,
              ),
            }
          : d,
    ...
    setDraft((prev) => ({
      ...
      dimensions: prev.dimensions.map((d, i) =>
        i === dimIndex
          ? {
              ...d,
              observations: d.observations.filter((_, j) => j !== obsIndex),
            }
          : d,
      ),
    }));
  ...
  const addDimQuickWin = (di) =>
    setDraft((prev) => ({
      ...prev,
      dimensions: prev.dimensions.map((d, i) =>
        i === di ? { ...d, quickWins: [...(d.quickWins || []), ""] } : d,
      ),
    }));
    ...
    setDraft((prev) => ({
      ...
      dimensions: prev.dimensions.map((d, i) =>
        i === di
          ? {
              ...d,
              prioritizedActions: (d.prioritizedActions || []).filter(
                (_, j) => j !== ai,
              ),
            }
          : d,
  ...
  const setCompetitorGap = (gi, value) =>
    setDraft((prev) => ({
      ...
      competitorInsights: {
        ...
        gaps: (prev.competitorInsights?.gaps || []).map((g, j) =>
          j === gi ? value : g,
        ),
      },
    }));
  ...
  const setRoadmapItem = (phase, ri, value) =>
    setDraft((prev) => ({
      ...
      roadmap: {
        ...
        [phase]: (prev.roadmap?.[phase] || []).map((it, j) =>
          j === ri ? value : it,
        ),
      },
    }));
  ...
  const setTool = (ti, value) =>
    setDraft((prev) => ({
      ...prev,
      toolRecommendations: (prev.toolRecommendations || []).map((t, j) =>
        j === ti ? value : t,
      ),
    }));
  const addTool = () =>
    setDraft((prev) => ({
      ...prev,
      toolRecommendations: [...(prev.toolRecommendations || []), ""],
    }));
  const removeTool = (ti) =>
    setDraft((prev) => ({
      ...prev,
      toolRecommendations: (prev.toolRecommendations || []).filter(
        (_, j) => j !== ti,
      ),
    }));

  const inputCls =
    "w-full bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-cyan-500";
  ...
  const Section = ({ id, title, children }) => (
    <div className="border border-slate-700 rounded-xl overflow-hidden mb-3">
      ...
      {openSection === id && (
        <div className="p-5 bg-slate-900/50 space-y-4">{children}</div>
      )}
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/70 backdrop-blur-sm p-6">
      <div className="w-full max-w-2xl bg-slate-900 rounded-2xl border border-slate-700 shadow-2xl my-auto">
        ...
        <div className="flex items-start justify-between p-6 border-b border-slate-700">
          ...
          <button
            onClick={onClose}
            className="ml-4 p-1.5 hover:bg-slate-700 rounded-lg transition-colors shrink-0"
          >
        ...
        <div className="p-6 max-h-[65vh] overflow-y-auto space-y-1 custom-scrollbar">
          ...
          {draft.dimensions.map((dim, di) => (
            <Section
              key={dim.id}
              id={`dim-${di}`}
              title={`${dim.dimension}. ${dim.name} — ${dim.score}/100`}
            >
              <div className="flex items-end gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={dim.score}
                    onChange={(e) =>
                      setDimField(di, "score", Number(e.target.value))
                    }
                ...
                <div className="flex-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDimField(di, "confidence", e.target.value)
                    }
              ...
              <div>
                ...
                {(dim.prioritizedActions || []).map((a, ai) => (
                  <div
                    key={ai}
                    className="border border-slate-700 rounded-lg p-3 mb-2 space-y-2 bg-slate-900/30"
                  >
                    <div className="flex gap-2">
                      <textarea
                        value={a.action || ""}
                        onChange={(e) =>
                          setDimAction(di, ai, "action", e.target.value)
                        }
                        rows={2}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <select
                        ...
                        onChange={(e) =>
                          setDimAction(di, ai, "effort", e.target.value)
                        }
                        className={inputCls}
                      >
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setDimAction(di, ai, "impact", e.target.value)
                        }
                        className={inputCls}
                      >
          ...
          <Section id="exec-summary" title="Executive Summary">
            <div>
              <label className={labelCls}>
                Narrative (3-5 paragraphs separated by blank lines)
              </label>
              <textarea
          ...
          <Section id="competitor" title="Competitor Gap Analysis">
            <div>
              <label className={labelCls}>
                Industry Benchmark (one sentence)
              </label>
              <textarea
                ...
                {(draft.roadmap?.[phase.key] || []).map((item, ri) => (
                  <div key={ri} className="flex gap-2 mb-2">
                    <textarea
                      value={item}
                      onChange={(e) =>
                        setRoadmapItem(phase.key, ri, e.target.value)
                      }
                      rows={1}
                      className={`${textareaCls} flex-1`}
                    />
                ...
                >
                  ...
                  <div>
                    ...
                    <input
                      value={issue.title}
                      onChange={(e) =>
                        setIssueField(ii, "title", e.target.value)
                      }
                      className={inputCls}
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setIssueField(ii, "description", e.target.value)
                      }
                      rows={2}
                      className={textareaCls}
                    />
                  ...
                  <div>
                    ...
                    <textarea
                      value={issue.fix}
                      onChange={(e) => setIssueField(ii, "fix", e.target.value)}
                      rows={2}
                      className={textareaCls}
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setIssueField(ii, "codeSnippet", e.target.value)
                      }
                      rows={3}
                      className={`${textareaCls} font-mono text-xs text-cyan-300`}
                    />
                  </div>
                </div>
              ))}
          ...
          {draft.improvements?.length > 0 && (
            <Section id="improvements" title="Prioritised Improvements">
              {draft.improvements.map((item, ii) => (
                <div
                  key={ii}
                  className="space-y-2 pb-3 border-b border-slate-700/50 last:border-0 last:pb-0"
                >
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                    #{item.rank || ii + 1}
                  </p>
                  <div>
                    ...
                    <input
                      value={item.title}
                      onChange={(e) =>
                        setImprovField(ii, "title", e.target.value)
                      }
                      className={inputCls}
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setImprovField(ii, "description", e.target.value)
                      }
                      rows={2}
                      className={textareaCls}
                    />
          ...
          {draft.signatureRecommendation && (
            <Section id="sig" title="Signature Recommendation">
              <div>
                <label className={labelCls}>Title</label>
                <input
                  value={draft.signatureRecommendation.title || ""}
                  onChange={(e) => setSigField("title", e.target.value)}
                  className={inputCls}
                />
              </div>
              <div>
                <label className={labelCls}>Description</label>
                <textarea
                  value={draft.signatureRecommendation.description || ""}
                  onChange={(e) => setSigField("description", e.target.value)}
                  rows={3}
                  className={textareaCls}
                />
              </div>
              <div>
                <label className={labelCls}>Code Snippet (optional)</label>
                <textarea
                  value={draft.signatureRecommendation.codeSnippet || ""}
                  onChange={(e) => setSigField("codeSnippet", e.target.value)}
                  rows={3}
                  className={`${textareaCls} font-mono text-xs text-cyan-300`}
                />
              </div>
        ...
        {/* Footer */}
        <div className="p-6 border-t border-slate-700 flex items-center justify-between gap-3 flex-wrap">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <button
              ...
              className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isGenerating ? (
                <>
            ...
            {onApprove && (
              <button
                ...
                className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:from-green-400 hover:to-emerald-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isApproving ? (
                  <>
                  ...
                  <>
                    <CheckCircle2 size={16} /> Approve & Notify User
                  </>
                )}
              </button>
            )}
